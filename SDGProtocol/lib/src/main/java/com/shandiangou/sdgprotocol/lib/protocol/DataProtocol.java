...
public class DataProtocol extends BasicProtocol implements Serializable {
    ...
    public int parseContentData(byte[] data) throws ProtocolException {
        if (parseLength(data) == data.length) {
            Log.e("linwb", "Length match...");
        } else {
            Log.e("linwb", "Length not match...");
        }
        ...
        Log.e("linwb", "pattion: " + pattion);
        pos += PATTION_LEN;

        //解析dtype
        dtype = data[pos] & 0xFF;
        Log.e("linwb", "dtype: " + dtype);
        pos += DTYPE_LEN;
